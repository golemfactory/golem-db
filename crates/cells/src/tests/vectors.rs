...
pub(super) const VECTORS: &[Vector] = &[
    ...
    // Signed values are stored sign-flipped: 0x7F FF… is -1, 0x80 00… is 0.
...
pub(super) const BAD_VECTORS: &[BadVector] = &[
    ...
    // -- content: floats with no single byte form, in stored form -------
    ("f32 quiet NaN",           &[0x18, 0xFF, 0xC0, 0, 0],                CellParseError::FloatNaN),
    ("f32 -0.0",                &[0x18, 0x7F, 0xFF, 0xFF, 0xFF],          CellParseError::NegativeZero),
    ("f64 quiet NaN",           &[0x19, 0xFF, 0xF8, 0, 0, 0, 0, 0, 0],    CellParseError::FloatNaN),
    ("f64 signalling NaN",      &[0x19, 0xFF, 0xF0, 0, 0, 0, 0, 0, 1],    CellParseError::FloatNaN),
    ...
    // -- field-only types cannot be indexable --------------------------
    ("bytes, idx bit set",      &[0x83, 0x00, 0x00, 0x00, 0x00],          CellParseError::NotIndexable(CellType::Bytes)),
    #[cfg(feature = "custom_types")]
    ("custom, idx bit set",     &[0xFF, 0x00, 0x00, 0x00, 0x01, 1],       CellParseError::NotIndexable(CellType::Custom(CustomTypeId::new(127).unwrap()))),
